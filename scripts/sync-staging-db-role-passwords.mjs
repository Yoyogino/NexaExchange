...
const databaseSql = [
  ...
  "DO $nexa$",
  "DECLARE application_object record;",
  "BEGIN",
  "  FOR application_object IN",
  ...
  `    WHERE namespace.nspname = 'public' AND pg_get_userbyid(object.relowner) = ${sqlLiteral(activeOwner)}`,
  "      AND object.relkind IN ('r', 'p', 'S', 'v', 'm', 'f')",
  "  LOOP",
  "    EXECUTE format(",
  "      'ALTER %s %I.%I OWNER TO nexa_migrator',",
  "      CASE application_object.relkind",
  "        WHEN 'S' THEN 'SEQUENCE' WHEN 'v' THEN 'VIEW' WHEN 'm' THEN 'MATERIALIZED VIEW'",
  "        WHEN 'f' THEN 'FOREIGN TABLE' ELSE 'TABLE'",
  "      END, application_object.nspname, application_object.relname",
  "    );",
  "  END LOOP;",
  "END",
  "$nexa$;",
